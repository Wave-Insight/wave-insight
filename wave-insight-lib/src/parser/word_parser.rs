...
pub enum ParserType {
    ModuleDefine(String, u32),
    EndModule,
    SignalDefine(String, u32),
    AssignLeft(String, u32),
    AssignRight(String, u32),
    EndAssign,
    SubModuleDefine(String, u32),
    SubModuleUse(String, u32),
}
...
pub fn state_update(word: (String,LastType), line_idx: u32, state: (State,i32)) -> ((State,i32),Option<ParserType>) {
    match (&word.0[..],word.1,state.0,state.1) {
        ("module",_,State::OutOfModule,_) => {((State::FindingModule,state.1),None)},
        (module_name,_,State::FindingModule,_) => {((State::ModuleBody,state.1),Some(ParserType::ModuleDefine(module_name.to_string(), line_idx)))},
        ...
        (signal_name,_,State::SignalDefine,_) => {((State::ModuleBody,state.1),Some(ParserType::SignalDefine(signal_name.to_string(), line_idx)))},
        ...
        (left,LastType::Normal,State::AssignLeft,_) => {((State::AssignLeft,state.1),Some(ParserType::AssignLeft(left.to_string(), line_idx)))},
        (right,LastType::Normal,State::AssignRight,_) => {((State::AssignRight,state.1),Some(ParserType::AssignRight(right.to_string(), line_idx)))},
        ...
        (left,LastType::Normal,State::AlwaysLeft,_) => {((State::AlwaysLeft,state.1),Some(ParserType::AssignLeft(left.to_string(), line_idx)))},
        ("=",_,State::AlwaysLeft,_) => {((State::AlwaysRight,state.1),None)},
        ("<=",_,State::AlwaysLeft,_) => {((State::AlwaysRight,state.1),None)},
        (right,LastType::Normal,State::AlwaysRight,_) => {((State::AlwaysRight,state.1),Some(ParserType::AssignRight(right.to_string(), line_idx)))},
        ...
        (define_name,LastType::Normal,State::ModuleBody,_) => {((State::SubModuleName,state.1),Some(ParserType::SubModuleDefine(define_name.to_string(), line_idx)))},
        (module_name,_,State::SubModuleName,_) => {((State::SubModuleBody,state.1),Some(ParserType::SubModuleUse(module_name.to_string(), line_idx)))},
