...
pub fn state_update(word: (String,LastType), state: (State,i32)) -> ((State,i32),Option<(ParserType,String)>) {
    match (&word.0[..],word.1,state.0,state.1) {
        ...
        (w,_,State::AssignRight,_) if w.ends_with(';') => {((State::ModuleBody,state.1),Some((ParserType::EndAssign,"".to_string())))},
        ...
        (w,_,State::AlwaysRight,_) if w.ends_with(';') => {((State::AlwaysLeft,state.1),Some((ParserType::EndAssign,"".to_string())))},

        ("endmodule",_,State::ModuleBody,_) => {((State::OutOfModule,0),Some((ParserType::EndModule,"".to_string())))},
